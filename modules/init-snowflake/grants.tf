...
resource snowflake_database_grant db_usage_grant {
  ...
  database_name = snowflake_database.db[each.key].name
  privilege = "USAGE"
...
resource snowflake_warehouse_grant wh_operate_grant {
  ...
  warehouse_name = snowflake_warehouse.wh[each.key].name
  privilege = "OPERATE"
...
resource snowflake_resource_monitor_grant resource_monitor_grant {
  ...
  monitor_name = snowflake_resource_monitor.monitor[each.key].name
  privilege = "MONITOR"
